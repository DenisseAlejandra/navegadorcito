...
class PerfilAlumnoController extends Controller{
    ...
    public function asignaturasActuales(String $rut){
    	$todas = MatriculaInstanciaCurso::join('instancia_cursos','matricula_instancia_cursos.instanciaCurso_id' , '=', 'instancia_cursos.id')->join('cursos', 'instancia_cursos.curso_id' , '=' , 'cursos.id')->select('matricula_instancia_cursos.alumno_id','instancia_cursos.agno','instancia_cursos.semestre', 'matricula_instancia_cursos.estadoMatricula_id', 'cursos.sigla', 'cursos.nombre', 'cursos.id' )->get();
